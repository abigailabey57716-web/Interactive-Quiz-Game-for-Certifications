...
(define-constant err-timeout u110)
...
(define-map quiz-time-limit {qid: uint} {limit: uint})
...
(define-public (set-quiz-time-limit (quiz-id uint) (limit uint))
(begin
    (try! (ensure-owner))
    (asserts! (is-some (map-get? quizzes {id: quiz-id})) (err err-not-found))
    (map-set quiz-time-limit {qid: quiz-id} {limit: limit})
    (ok true)
  )
)

(define-read-only (get-quiz-time-limit (quiz-id uint))
  (ok (map-get? quiz-time-limit {qid: quiz-id}))
)
...
(define-public (answer-next (quiz-id uint) (answer-index uint))
  (begin
    (let ((a (unwrap! (map-get? attempts {qid: quiz-id, user: tx-sender}) (err err-not-found)))
          (start-time (unwrap! (map-get? attempt-start-times {qid: quiz-id, user: tx-sender}) (err err-not-found)))
          (limit-entry (map-get? quiz-time-limit {qid: quiz-id})))
      (let ((elapsed (- stacks-block-height (get start-block start-time)))
            (time-ok (if (is-some limit-entry)
                         (let ((limit-value (get limit (unwrap-panic limit-entry))))
                           (if (> limit-value u0) (<= elapsed limit-value) true))
                         true)))
        (asserts! time-ok (err err-timeout))
